...
- **2.5 Docking and reference-pose RMSD** — AutoDock Vina 1.2.5 via WSL,
  ...
  pose-to-best-pose RMSD, which is not a reference-pose metric).
...
| --- | --- | --- | --- |
| Fig. 1 | Structural inventory disposition (clean/contextual/review-required) | `reports/generated/figures/figure-01-structural-inventory` | **exists**, regenerate with current 19-attempt totals |
| Fig. 2 | Strict preparation outcomes (prepared/failed by class) | `figure-02-strict-preparation` | **exists**, regenerate |
| Fig. 3 | Reference-pose RMSD vs. Vina score, all 4 completed cases | `figure-03-reference-pose-outcomes` | **exists but only shows the original 2-case data by default** — must be regenerated pointing at the merged rmsd set (already wired via `render_audit_figures.py`'s contextual-batch glob); verify the 3CJO/3PTB points appear before using in the manuscript |
| Fig. 4 | Evidence flow (candidates → retrieved → classified → prepared → docked) | `figure-04-evidence-flow` | **exists**, regenerate |
| Fig. 5 (new) | 3D binding-site renders: docked top pose vs. experimental pose, for each of the 4 completed cases (1STP, 3D4Q, 3CJO, 3PTB) | not yet produced | **gap — highest-value addition for a JMM reviewer; see below** |
...
| Table 2 | All 19 preparation attempts (case, PDB, batch, outcome, failure class) | already exists as the table in `reports/STRICT-PREPARATION-SUMMARY-v0.1.md` | reuse directly |
| Table 3 | 4 completed docking cases (case, PDB, ligand, top-score affinity, top-score RMSD, best RMSD) | already exists as the table in `reports/AUDIT-SYNTHESIS-v0.1.md` | reuse directly |

## Gaps to close before submission

1. **Figure 5 (3D binding-site renders) does not exist yet.** This is the
   single most valuable addition for "buena cantidad de imágenes" — a JMM
   reviewer will expect to see the docked pose overlaid on the experimental
   pose for at least the headline cases. Needs a rendering script (PyMOL or
   3Dmol/py3Dmol headless) reading the already-computed alignment from
   `calculate_reference_pose_rmsd.py`'s output.
2. **Regenerate figures 1-4 against the current 19-attempt state** — the
