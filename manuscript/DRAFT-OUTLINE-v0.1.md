...
| Fig. 5 | 3D receptor cartoon renders (real 3Dmol.js, not matplotlib), one per completed docking case (1STP, 3D4Q, 3CJO, 3PTB, 1HVR, 1IEP, 1B9V) | `reports/generated/figures/platform-3d/<case>-<PDB>-platform-3d-white.png` — PNG captures taken directly from the scAMH platform's "Docking Reference Audit" project's `ThreeDmolDockingViewer` (real WebGL cartoon rendering of the same strict-prepared receptor and Vina poses used throughout this project), white background (a toggle was added to `DockingAuditScreen.tsx` for this — dark stays the default for interactive/personal use) | **done, superseded matplotlib version** — the earlier `scripts/render_binding_site_figures.py` matplotlib renders (thin-line cartoon, visually weak) were replaced with these platform captures once the reviewer-facing quality gap was flagged; matplotlib script kept in the repo for the receptor/pose coordinate cross-check it still documents, but is no longer the Fig. 5 source. Caption should still note that a single static 3D projection can visually understate or overstate depth separation — the RMSD number, not the image, is the quantitative claim. Capture is manual (browser), not scripted; if regenerating, open the platform's Docking Reference Audit → Visor 3D screen, toggle "Fondo blanco (para figuras)", select each case, and export the canvas. |
