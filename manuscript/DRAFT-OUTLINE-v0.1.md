...
| Fig. 5 | 3D receptor cartoon renders (real 3Dmol.js, not matplotlib), zoomed to the binding pocket with interface-contact residues highlighted, one per completed docking case (1STP, 3D4Q, 3CJO, 3PTB, 1HVR, 1IEP, 1B9V) | `reports/generated/figures/platform-3d/<case>-<PDB>-platform-3d-white.png` — PNG captures from the scAMH platform's "Docking Reference Audit" project's `ThreeDmolDockingViewer`, white background. The viewer now highlights the same contact residues as Table 4 / Fig. 7 directly on the receptor cartoon (green = H-bond candidate, red = other contact ≤4.5 Å, data from `data/interface_contacts_by_case.json` — derived from the already-verified `interface_contacts_top_pose.csv`/`hydrogen_bonds_top_pose.csv`, not recomputed) and frames the camera on the pocket instead of the whole receptor | **done** — superseded both the original matplotlib version (thin-line cartoon, visually weak) and the first plain-spectrum platform capture (no highlight, whole-receptor view, hard to see the ligand). Caption should still note that a single static 3D projection can visually understate or overstate depth separation — the RMSD number, not the image, is the quantitative claim. Capture is manual (browser), not scripted; if regenerating, open the platform's Docking Reference Audit → Visor 3D screen, keep "Resaltar contactos + zoom al bolsillo" and "Fondo blanco (para figuras)" on, select each case, and export the canvas. |
| Fig. 6 | Workflow/protocol diagram (candidate registration → classification → preparation → docking → RMSD verification), each stage annotated with its real already-reported count | `reports/generated/figures/figure-06-workflow.svg`, generated by `scripts/render_workflow_diagram.py` | **done** — reader-orientation diagram only, no new data claim; every number on it is copied from AUDIT-SYNTHESIS/STRICT-PREPARATION-SUMMARY/REPRODUCIBILITY-VERIFICATION, not recomputed |
